...
void MenuBase::handleNumpad(const String &data) {
    ...
    if (action == "cancel") { resetInputState(); requestTgUpdate(true); return; }
    if (action == "clear") { state.inputBuffer = ""; requestTgUpdate(true); return; }
    if (action == "dot") {
        if (state.inputBuffer.indexOf('.') == -1) state.inputBuffer += ".";
        requestTgUpdate(true); return;
    }
...
String MenuBase::generateStatusText() {
    ...
    if (state.waitingForTargetT || state.waitingForHysteresis || state.waitingForPauseTemp || state.waitingForPauseDur) {
        statusText = "🔢 *Ввод значения*\n\n";
        if (state.waitingForTargetT) statusText += "🌡 Целевая температура куба (T1)\n";
        else if (state.waitingForHysteresis) statusText += "⚙️ Гистерезис (T2)\n";
        else if (state.waitingForPauseTemp) statusText += "🍺 Затирание (Шаг 1/2): Температура\n";
        else if (state.waitingForPauseDur) statusText += "🍺 Затирание (Шаг 2/2): Время (мин)\n";
...
String MenuBase::buildMessageJson(const String &text, int msgId) {
    ...
    if (msgId != 0) doc["message_id"] = msgId;
...
void MenuBase::processCallback(const String &data, int msgId) {
    if (data.startsWith("/num_")) { handleNumpad(data); return; }
    if (data == "/status") { state.mashMode = false; requestTgUpdate(true); return; }
    if (data == "/m_menu") { state.mashMode = true; requestTgUpdate(true); return; }
    if (data == "/delete_msg") {
        JsonDocument doc;
        doc["chat_id"] = config.chat_id;
